...
static void pcmd_set_promisc_mode_parsed(void *parsed_result,
					...
					void *allports)
{
	...
	if (allports) {
		RTE_ETH_FOREACH_DEV(i)
			ret = eth_set_promisc_mode(i, enable);
	} else {
		ret = eth_set_promisc_mode(res->port_num, enable);
	}
	if (ret) {
		if (enable)
		...
		printf("Error while updating promisc mode");
}
