...
ext_mbuf_create(struct rte_mempool *mbuf_pool, int pkt_len,
		int nb_segs, const uint8_t *input_text)
{
	...
	for (i = 0; size > 0; i++) {
		...
		memcpy(RTE_PTR_ADD(ext_buf_addr, 0), RTE_PTR_ADD(input_text, data_off), data_len);
...
static int
test_ipsec_proto_sgl_ext_mbuf(const void *data __rte_unused)
{
	struct crypto_testsuite_params *ts_params = &testsuite_params;
	struct rte_cryptodev_info dev_info;
