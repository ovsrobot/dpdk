...
test_rx_from_file(void)
{
	...
	receive_packets(port_id, mbufs, RTE_DIM(mbufs), &received);
	TEST_ASSERT_EQUAL(received, RTE_DIM(mbufs),
			  "Received %u packets, expected %lu",
			  received, RTE_DIM(mbufs));
...
test_tx_drop(void)
{
	...
	TEST_ASSERT(alloc_test_mbufs(mbufs, RTE_DIM(mbufs)) == 0,
		    "Failed to allocate mbufs");

	TEST_ASSERT(rte_eth_stats_reset(port_id) == 0,
		    "Failed to reset stats");
	nb_tx = rte_eth_tx_burst(port_id, 0, mbufs, RTE_DIM(mbufs));

	/* Packets should be accepted even in drop mode */
	TEST_ASSERT_EQUAL(nb_tx, RTE_DIM(mbufs),
			  "Drop mode TX: %d/%lu accepted",
			  nb_tx, RTE_DIM(mbufs));
