...
test_simple(void)
{
	...
	tmp = tmpfile();
	TEST_ASSERT(tmp != NULL, "Tmpfile() failed!");
	...
	for (unsigned int i = 0; i < TOTAL_PACKETS; i++)
		rte_dissect_mbuf(tmp, &mb, 0);

	rewind(tmp);
	lines = count_lines(tmp);

	TEST_ASSERT_EQUAL(lines, TOTAL_PACKETS,
			  "Expected %d lines but got %d",
			  TOTAL_PACKETS, lines);
...
test_truncated(void)
{
	...
	uint16_t dst_port = rte_cpu_to_be_16(RTE_VXLAN_DEFAULT_PORT);
	int lines;
	FILE *tmp;
	...
	for (unsigned int i = 0; i < TOTAL_PACKETS; i++) {
		...
		rte_dissect_mbuf(tmp, &mb, snaplen);
	}
...
test_fuzz(void)
{
	...
	uint8_t buf[RTE_MBUF_DEFAULT_BUF_SIZE];
	uint32_t data_len = PACKET_LEN;
	const uint16_t dst_port = rte_cpu_to_be_16(rte_rand_max(1024));
	const uint16_t src_port = rte_cpu_to_be_16(rte_rand());
	int lines;
	FILE *tmp;
