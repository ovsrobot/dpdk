...
struct memstat {
	struct {
		RTE_ATOMIC(uint64_t) nb_call;
		RTE_ATOMIC(uint64_t) nb_fail;
		RTE_ATOMIC(uint64_t) sz;
	} alloc;
	struct {
		RTE_ATOMIC(uint64_t) nb_call;
		RTE_ATOMIC(uint64_t) nb_fail;
...
struct __rte_cache_aligned memtank_arg {
	struct rte_memtank *mt;
	union {
		struct main_args main;
...
test_alloc1(size_t sz, void *p)
{
	...
	ms = p;
	buf = alloc_func(sz);
	rte_atomic_fetch_add_explicit(&ms->alloc.nb_call, 1,
		rte_memory_order_relaxed);
	if (buf != NULL) {
		memset(buf, 0, sz);
		 rte_atomic_fetch_add_explicit(&ms->alloc.sz, sz,
			rte_memory_order_relaxed);
	} else
		rte_atomic_fetch_add_explicit(&ms->alloc.nb_fail, 1,
			rte_memory_order_relaxed);
...
test_free1(void *buf, void *p)
{
	...
	free_func(buf);
	rte_atomic_fetch_add_explicit(&ms->free.nb_call, 1,
			rte_memory_order_relaxed);
	if (buf == NULL)
		rte_atomic_fetch_add_explicit(&ms->free.nb_fail, 1,
			rte_memory_order_relaxed);
}
...
memstat_dump(FILE *f, struct memstat *ms)
{
	...
	nb_alloc = rte_atomic_load_explicit(&ms->alloc.nb_call,
			rte_memory_order_relaxed) -
		rte_atomic_load_explicit(&ms->alloc.nb_fail,
			rte_memory_order_relaxed);
	alloc_sz = rte_atomic_load_explicit(&ms->alloc.sz,
		rte_memory_order_relaxed) / nb_alloc;
	nb_alloc -= rte_atomic_load_explicit(&ms->free.nb_call,
		rte_memory_order_relaxed) -
		rte_atomic_load_explicit(&ms->free.nb_fail,
		rte_memory_order_relaxed);
	alloc_sz *= nb_alloc;
	mut = (alloc_sz == 0) ? 1 :
	...
	fprintf(f, "\t\tnb_call=%" PRIu64 ",\n",
		rte_atomic_load_explicit(&ms->alloc.nb_call,
		rte_memory_order_relaxed));
	fprintf(f, "\t\tnb_fail=%" PRIu64 ",\n",
		rte_atomic_load_explicit(&ms->alloc.nb_fail,
		rte_memory_order_relaxed));
	fprintf(f, "\t\tsz=%" PRIu64 ",\n",
		rte_atomic_load_explicit(&ms->alloc.sz,
		rte_memory_order_relaxed));
	fprintf(f, "\t},\n");
	fprintf(f, "\tfree={\n");
	fprintf(f, "\t\tnb_call=%" PRIu64 ",\n",
		rte_atomic_load_explicit(&ms->free.nb_call,
		rte_memory_order_relaxed));
	fprintf(f, "\t\tnb_fail=%" PRIu64 ",\n",
		rte_atomic_load_explicit(&ms->free.nb_fail,
		rte_memory_order_relaxed));
	...
	uint8_t check, uint8_t fill)
{
	...
	for (i = 0; i != num; i++) {
		...
		if (j != sz) {
			...
			printf("%u-th offset, expected: %#x, result: %#x;\n",
				j, check, buf[j]);
...
test_memtank_main(void *arg)
{
	...
	for (cl = 0, i = 0; cl < ma->main.run_cycles;
			cl += tm2 - tm0, i++)  {

		tm0 = rte_rdtsc_precise();

		if (ma->main.flags & MAIN_FLAG_SHRINK) {
		...
		if (ma->main.flags & MAIN_FLAG_GROW) {
...
static void
fill_main_args(struct main_args *ma, uint32_t flags)
{
	uint64_t tm;
	...
	struct memtank_arg arg[], const char *tname)
{
	...
	/* launch on main */
	lc = rte_lcore_id();
	arg[lc].main.run_cycles = CLEANUP_TIME * rte_get_timer_hz();
	test_memtank_main(&arg[lc]);
...
test_memtank_mt(const char *tname, uint32_t alloc_flags, uint32_t free_flags)
{
	...
	lc = rte_lcore_id();
	arg[lc].mt = mt;
	fill_main_args(&arg[lc].main,
		MAIN_FLAG_GROW | MAIN_FLAG_SHRINK);
	test_memtank_main(&arg[lc]);
	...
	/* run cleanup on all worker cores */
	if (rc == 0)
