...
def show_numa_pages():
    ...
    for numa_path in glob.glob('/sys/devices/system/node/node*'):
        ...
        for hdir in os.listdir(path):
            ...
            if pages > 0:
                pg_sz = fmt_memsize(
                    hdir[10:-2])  # slice out of hugepages-NNNkB
                print('{:<4} {:<5} {}'.format(node, pages, pg_sz))
...
def show_non_numa_pages():
    ...
    for hdir in os.listdir(path):
        pages = get_hugepages(path + '/' + hdir)
        if pages > 0:
            pg_sz = fmt_memsize(int(hdir[10:-2]))
            print('{:<5} {}'.format(pages, pg_sz))
...
""".format(argv0=argv0))
    ...
    parser.add_argument(
        ...
        '-n',
        action='store',
        help='select numa node to reserve pages on')
    parser.add_argument(
        '--pagesize',
        '-p',
        action='store',
