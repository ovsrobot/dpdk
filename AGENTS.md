...
should use `rte_zmalloc_socket()` rather than plain `rte_malloc()`:
zero-initialization avoids stale descriptor bugs, the `_socket` variant
ensures NUMA-local allocation, and hugepage backing makes the memory
visible to secondary processes.
