...
### LTS (Long Term Stable) Releases

Any DPDK release whose minor version is `.11` is an LTS release
(e.g., 19.11, 20.11, 21.11, 22.11, 23.11, 24.11, 25.11, …).
To determine whether a version is LTS, extract the base
`MAJOR.MINOR` (e.g., `23.11` from `23.11.1`) and check whether the
minor component is `11`.  An explicit `-lts` suffix (e.g.,
`23.11-lts`) also indicates an LTS release.
...
- Changes to ABI on LTS release (see LTS rules above)
