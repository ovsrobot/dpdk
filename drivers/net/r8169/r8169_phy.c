...
rtl_set_phy_mcu_patch_request(struct rtl_hw *hw)
{
	...
	if (!bool_success)
		PMD_INIT_LOG(NOTICE, "rtl_set_phy_mcu_patch_request fail.");
...
rtl_clear_phy_mcu_patch_request(struct rtl_hw *hw)
{
	...
	if (!bool_success)
		PMD_INIT_LOG(NOTICE, "rtl_clear_phy_mcu_patch_request fail.");
...
rtl_xmii_reset_enable(struct rtl_hw *hw)
{
	...
	rtl_mdio_direct_write_phy_ocp(hw, 0xA5D4, rtl_mdio_direct_read_phy_ocp(hw,
	                              0xA5D4) & ~(RTK_ADVERTISE_2500FULL | RTK_ADVERTISE_5000FULL));
