...
static int
nfp_flower_enable_pf_vnic_service(struct nfp_app_fw_flower *app_fw_flower)
{
	int ret;
	uint32_t service_id;
	struct rte_service_spec pf_vnic_service = {
		.name         = "flower_pf_vnic_service",
		.callback     = nfp_flower_pf_vnic_service,
	};

	/* Pass a pointer to the flower app to the service */
	pf_vnic_service.callback_userdata = (void *)app_fw_flower;

	/* Register the flower services */
	ret = rte_service_component_register(&pf_vnic_service, &service_id);
	if (ret != 0) {
		PMD_INIT_LOG(ERR, "Could not register flower pf vnic service");
		return -EINVAL;
	}
	...
	if (ret != 0) {
		PMD_INIT_LOG(ERR, "Could not map flower pf vnic service");
		return -EINVAL;
	}
...
static int
nfp_flower_enable_ctrl_vnic_service(struct nfp_app_fw_flower *app_fw_flower)
{
	int ret;
	uint32_t service_id;
	struct rte_service_spec ctrl_vnic_service = {
		.name         = "flower_ctrl_vnic_service",
		.callback     = nfp_flower_ctrl_vnic_service,
	};

	/* Pass a pointer to the flower app to the service */
	ctrl_vnic_service.callback_userdata = (void *)app_fw_flower;

	/* Register the flower services */
	ret = rte_service_component_register(&ctrl_vnic_service, &service_id);
	if (ret != 0) {
		PMD_INIT_LOG(ERR, "Could not register flower ctrl vnic service");
		return -EINVAL;
	}

	PMD_INIT_LOG(INFO, "Flower ctrl vnic service registered");

	/* Map them to available service cores*/
	ret = nfp_map_service(service_id);
	if (ret != 0) {
		PMD_INIT_LOG(ERR, "Could not map flower ctrl vnic service");
		return -EINVAL;
	}
...
nfp_init_app_fw_flower(struct nfp_pf_dev *pf_dev)
{
	...
	/* Start up flower ctrl vnic service */
	ret = nfp_flower_enable_ctrl_vnic_service(app_fw_flower);
	if (ret != 0) {
		PMD_INIT_LOG(ERR, "Enable nfp ctrl vnic service failed.");
		ret = -EINVAL;
		goto ctrl_cpp_area_cleanup;
	}

	/* Start up flower pf vnic service */
	ret = nfp_flower_enable_pf_vnic_service(app_fw_flower);
	if (ret != 0)
		PMD_INIT_LOG(WARNING, "Enable nfp pf vnic service failed.");
