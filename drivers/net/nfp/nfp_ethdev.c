...
nfp_net_close(struct rte_eth_dev *dev)
{
	...
	app_hw_nic->ports[hw->idx] = NULL;
	rte_eth_dev_release_port(dev);

	for (i = 0; i < app_hw_nic->total_phyports; i++) {
		/* Check to see if ports are still in use */
		if (app_hw_nic->ports[i])
			return 0;
	}
		...
		struct nfp_eth_table *nfp_eth_table)
{
	...
	app_hw_nic = rte_zmalloc("nfp_app_fw_nic", sizeof(*app_hw_nic), 0);
	if (app_hw_nic == NULL)
		return -ENOMEM;

	/* Point the app_fw_priv pointer in the PF to the coreNIC app */
	pf_dev->app_fw_priv = app_hw_nic;
	...
	app_hw_nic->total_phyports = total_vnics;
	app_hw_nic->pf_dev = pf_dev;
	if (total_vnics > 1)
		app_hw_nic->multiport = true;

	/* Map the symbol table */
	pf_dev->ctrl_bar = nfp_rtsym_map(pf_dev->sym_tbl, "_pf0_net_bar0",
			app_hw_nic->total_phyports * 32768, &pf_dev->ctrl_area);
	if (pf_dev->ctrl_bar == NULL) {
...
port_cleanup:
	for (i = 0; i < app_hw_nic->total_phyports; i++) {
		if (app_hw_nic->ports[i] && app_hw_nic->ports[i]->eth_dev) {
			struct rte_eth_dev *tmp_dev;
			tmp_dev = app_hw_nic->ports[i]->eth_dev;
			rte_eth_dev_release_port(tmp_dev);
			app_hw_nic->ports[i] = NULL;
		}
	}
	nfp_cpp_area_free(pf_dev->ctrl_area);
app_cleanup:
...
nfp_pf_init(struct rte_pci_device *pci_dev)
{
	...
	app_hw_id = nfp_rtsym_read_le(sym_tbl, "_pf0_net_app_id", &err);
	if (err != 0) {
		PMD_INIT_LOG(ERR, "Couldn't read app_hw_id from fw");
		ret = -EIO;
	...
	if (ret != 0)
		PMD_INIT_LOG(WARNING, "Enable cpp service failed.");
