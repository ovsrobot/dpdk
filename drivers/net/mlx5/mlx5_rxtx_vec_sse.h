	...
	for (pos = 0; pos < mcqe_n; ) {
		...
		if (!rxq->cqe_comp_layout)
			for (i = 0; i < MLX5_VPMD_DESCS_PER_LOOP; ++i)
				if (likely(pos + i < mcqe_n))
					rte_prefetch0((void *)RTE_IGNORE_CAST_QUAL(cq + pos + i));
		/* A.1 load mCQEs into a 128bit register. */
		mcqe1 = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL
			(&mcq[pos % 8]));
		mcqe2 = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL
			(&mcq[pos % 8 + 2]));
		...
		if (!rxq->cqe_comp_layout) {
			if (!(pos & 0x7) && pos < mcqe_n) {
				if (pos + 8 < mcqe_n)
					rte_prefetch0((void *)RTE_IGNORE_CAST_QUAL
						(cq + pos + 8));
		 ...
		 uint16_t pkts_n, uint64_t *err, uint64_t *comp)
{
	...
	for (pos = 0;
		...
		cqes[3] = _mm_loadl_epi64((__m128i *)RTE_IGNORE_CAST_QUAL
					   (&cq[pos + p3].sop_drop_qpn));
		rte_compiler_barrier();
		p2 = _mm_extract_epi16(p, 2);
		cqes[2] = _mm_loadl_epi64((__m128i *)RTE_IGNORE_CAST_QUAL
		...
		cqes[1] = _mm_loadl_epi64((__m128i *)RTE_IGNORE_CAST_QUAL
					   (&cq[pos + p1].sop_drop_qpn));
		rte_compiler_barrier();
		cqes[0] = _mm_loadl_epi64((__m128i *)RTE_IGNORE_CAST_QUAL
					   (&cq[pos].sop_drop_qpn));
		...
		cqe_tmp2 = _mm_load_si128((__m128i *)RTE_IGNORE_CAST_QUAL
			(&cq[pos + p3]));
		cqe_tmp1 = _mm_load_si128((__m128i *)RTE_IGNORE_CAST_QUAL
			(&cq[pos + p2]));
		cqes[3] = _mm_blendv_epi8(cqes[3], cqe_tmp2, blend_mask);
		cqes[2] = _mm_blendv_epi8(cqes[2], cqe_tmp1, blend_mask);
		cqe_tmp2 = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL
			(&cq[pos + p3].csum));
		cqe_tmp1 = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL
			(&cq[pos + p2].csum));
		cqes[3] = _mm_blend_epi16(cqes[3], cqe_tmp2, 0x30);
		cqes[2] = _mm_blend_epi16(cqes[2], cqe_tmp1, 0x30);
		cqe_tmp2 = _mm_loadl_epi64((__m128i *)RTE_IGNORE_CAST_QUAL
			(&cq[pos + p3].rsvd4[2]));
		cqe_tmp1 = _mm_loadl_epi64((__m128i *)RTE_IGNORE_CAST_QUAL
			(&cq[pos + p2].rsvd4[2]));
