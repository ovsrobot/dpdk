...
fm10k_rxq_rearm(struct fm10k_rx_queue *rxq)
{
	...
	if (rte_mempool_get_bulk(rxq->mp,
		...
		for (i = 0; i < RTE_FM10K_RXQ_REARM_THRESH; i++) {
			...
			_mm_store_si128((__m128i *)RTE_IGNORE_CAST_QUAL(&rxdp[i].q),
						dma_addr0);
		}
	...
	for (i = 0; i < RTE_FM10K_RXQ_REARM_THRESH; i += 2, mb_alloc += 2) {
		...
		*(uint64_t *)p1 = rxq->mbuf_initializer;
		...
		_mm_store_si128((__m128i *)RTE_IGNORE_CAST_QUAL(&rxdp++->q),
				dma_addr0);
		_mm_store_si128((__m128i *)RTE_IGNORE_CAST_QUAL(&rxdp++->q),
				dma_addr1);
		...
		uint16_t nb_pkts, uint8_t *split_packet)
{
	...
	for (pos = 0, nb_pkts_recd = 0; pos < nb_pkts;
		...
		/* A.1 load desc[3] */
		descs0[3] = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL(rxdp + 3));
		rte_compiler_barrier();
		...
		/* A.1 load desc[2-0] */
		descs0[2] = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL(rxdp + 2));
		rte_compiler_barrier();
		descs0[1] = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL(rxdp + 1));
		rte_compiler_barrier();
		descs0[0] = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL(rxdp));
