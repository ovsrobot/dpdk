...
i40e_rxq_rearm(struct i40e_rx_queue *rxq)
{
	...
	if (unlikely(rte_mempool_get_bulk(rxq->mp,
		...
		if (rxq->rxrearm_nb + RTE_I40E_RXQ_REARM_THRESH >=
			...
			for (i = 0; i < RTE_I40E_DESCS_PER_LOOP; i++) {
				...
				vst1q_u64((uint64_t *)RTE_IGNORE_CAST_QUAL
						(&rxdp[i].read), zero);
			}
		}
	...
	for (i = 0; i < RTE_I40E_RXQ_REARM_THRESH; i += 2, rxep += 2) {
		...
		vst1q_u64((uint64_t *)RTE_IGNORE_CAST_QUAL(&rxdp++->read), dma_addr0);

		paddr = mb1->buf_iova + RTE_PKTMBUF_HEADROOM;
		dma_addr1 = vdupq_n_u64(paddr);
		vst1q_u64((uint64_t *)RTE_IGNORE_CAST_QUAL(&rxdp++->read), dma_addr1);
	}
...
descs_to_fdir_32b(volatile union i40e_rx_desc *rxdp, struct rte_mbuf **rx_pkt)
{
	...
	desc0_qw23 = vld1q_u64
			((uint64_t *)RTE_IGNORE_CAST_QUAL(&(rxdp + 0)->wb.qword2));
	desc1_qw23 = vld1q_u64
			((uint64_t *)RTE_IGNORE_CAST_QUAL(&(rxdp + 1)->wb.qword2));
	desc2_qw23 = vld1q_u64
			((uint64_t *)RTE_IGNORE_CAST_QUAL(&(rxdp + 2)->wb.qword2));
	desc3_qw23 = vld1q_u64
			((uint64_t *)RTE_IGNORE_CAST_QUAL(&(rxdp + 3)->wb.qword2));
		   ...
		   uint16_t nb_pkts, uint8_t *split_packet)
{
	...
	for (pos = 0, nb_pkts_recd = 0; pos < nb_pkts;
		...
		/* A.1 load desc[3-0] */
		descs[3] =  vld1q_u64((uint64_t *)RTE_IGNORE_CAST_QUAL(rxdp + 3));
		descs[2] =  vld1q_u64((uint64_t *)RTE_IGNORE_CAST_QUAL(rxdp + 2));
		descs[1] =  vld1q_u64((uint64_t *)RTE_IGNORE_CAST_QUAL(rxdp + 1));
		descs[0] =  vld1q_u64((uint64_t *)RTE_IGNORE_CAST_QUAL(rxdp));

		/* Use acquire fence to order loads of descriptor qwords */
		rte_atomic_thread_fence(rte_memory_order_acquire);
		/* A.2 reload qword0 to make it ordered after qword1 load */
		descs[3] = vld1q_lane_u64((uint64_t *)RTE_IGNORE_CAST_QUAL
				(rxdp + 3), descs[3], 0);
		descs[2] = vld1q_lane_u64((uint64_t *)RTE_IGNORE_CAST_QUAL
				(rxdp + 2), descs[2], 0);
		descs[1] = vld1q_lane_u64((uint64_t *)RTE_IGNORE_CAST_QUAL
				(rxdp + 1), descs[1], 0);
		descs[0] = vld1q_lane_u64((uint64_t *)RTE_IGNORE_CAST_QUAL
				(rxdp), descs[0], 0);
		...
		struct rte_mbuf *pkt, uint64_t flags)
{
	...
	vst1q_u64((uint64_t *)RTE_IGNORE_CAST_QUAL(txdp), descriptor);
}
