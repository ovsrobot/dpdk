...
i40e_rxq_rearm(struct i40e_rx_queue *rxq)
{
	...
	for (i = 0; i < RTE_I40E_RXQ_REARM_THRESH; i += 2, rxep += 2) {
		...
		_mm_store_si128((__m128i *)RTE_IGNORE_CAST_QUAL
				(&rxdp++->read), dma_addr0);
		_mm_store_si128((__m128i *)RTE_IGNORE_CAST_QUAL
				(&rxdp++->read), dma_addr1);
	}
...
descs_to_fdir_32b(volatile union i40e_rx_desc *rxdp, struct rte_mbuf **rx_pkt)
{
	...
	desc0_qw23 = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL
			(&(rxdp + 0)->wb.qword2));
	desc1_qw23 = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL
			(&(rxdp + 1)->wb.qword2));
	desc2_qw23 = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL
			(&(rxdp + 2)->wb.qword2));
	desc3_qw23 = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL
			(&(rxdp + 3)->wb.qword2));
		   ...
		   uint16_t nb_pkts, uint8_t *split_packet)
{
	...
	for (pos = 0, nb_pkts_recd = 0; pos < nb_pkts;
		...
		/* A.1 load desc[3] */
		descs[3] = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL(rxdp + 3));
		...
		/* A.1 load desc[2-0] */
		descs[2] = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL(rxdp + 2));
		rte_compiler_barrier();
		descs[1] = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL(rxdp + 1));
		rte_compiler_barrier();
		descs[0] = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL(rxdp));
