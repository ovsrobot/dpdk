...
i40e_rxq_rearm(struct i40e_rx_queue *rxq)
{
	...
	if (rte_mempool_get_bulk(rxq->mp,
		...
		if (rxq->rxrearm_nb + RTE_I40E_RXQ_REARM_THRESH >=
			...
			for (i = 0; i < RTE_I40E_DESCS_PER_LOOP; i++) {
				...
				vec_st(dma_addr0, 0, (__vector unsigned long *)
	...
	for (i = 0; i < RTE_I40E_RXQ_REARM_THRESH; i += 2, rxep += 2) {
		...
		*(uint64_t *)p1 = rxq->mbuf_initializer;
		...
		vec_st(dma_addr0, 0, (__vector unsigned long *)
			RTE_IGNORE_CAST_QUAL(&rxdp++->read));
		vec_st(dma_addr1, 0, (__vector unsigned long *)
			RTE_IGNORE_CAST_QUAL(&rxdp++->read));
	}
		   ...
		   uint16_t nb_pkts, uint8_t *split_packet)
{
	...
	for (pos = 0, nb_pkts_recd = 0; pos < nb_pkts;
		...
		/* A.1 load desc[3] */
		descs[3] = *(__vector unsigned long *)RTE_IGNORE_CAST_QUAL(rxdp + 3);
		...
		*(__vector unsigned long *)&rx_pkts[pos] = mbp1;
		...
		/* A.1 load desc[2-0] */
		descs[2] = *(__vector unsigned long *)RTE_IGNORE_CAST_QUAL(rxdp + 2);
		rte_compiler_barrier();
		descs[1] = *(__vector unsigned long *)RTE_IGNORE_CAST_QUAL(rxdp + 1);
		rte_compiler_barrier();
		descs[0] = *(__vector unsigned long *)RTE_IGNORE_CAST_QUAL(rxdp);
	...
	struct rte_mbuf *pkt, uint64_t flags)
{
	...
	*(__vector unsigned long *)RTE_IGNORE_CAST_QUAL(txdp) = descriptor;
}
