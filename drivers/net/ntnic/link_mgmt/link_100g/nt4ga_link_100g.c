 ...
 */
static int _create_nim(adapter_info_t *drv, nthw_fpga_t *fpga, int port, bool enable)
{
	int res = 0;
...
static int _port_init(adapter_info_t *drv, nthw_fpga_t *fpga, int port)
{
	...
	if (adapter_id == NT_HW_ADAPTER_ID_NT200A01 && hw_id == 1) {
		...
		NT_LOG(ERR, ETHDEV, "%s: Unhandled AdapterId/HwId: %02x_hwid%d\n", __func__,
			adapter_id, hw_id);
		assert(0);
	}
...
static void read_nim_sensors(adapter_info_t *drv, int port)
{
	if (drv->nim_sensors[port] != NULL) {
		...
		if (t_spi) {
			for (struct nim_sensor_group *ptr = drv->nim_sensors[port]; ptr != NULL;
				ptr = ptr->next) {
				if (ptr->read)
					ptr->read(ptr, t_spi);
			}
...
static int _common_ptp_nim_state_machine(void *data)
{
	...
	while (monitor_task_is_running[adapter_no]) {
		...
		if (drv->adapter_sensors != NULL) {
			...
			if (t_spi) {
				for (struct nt_sensor_group *ptr = drv->adapter_sensors;
					ptr != NULL;
					ptr = ptr->next) {
					ptr->read(ptr, t_spi);
				}
		...
		for (i = 0; i < nb_ports; i++) {
			...
			if (link_info->port_action[i].port_lpbk_mode != last_lpbk_mode[i]) {
				...
				NT_LOG(INF, ETHDEV, "%s: Loopback mode changed=%u\n",
			...
			if (!new_link_state.nim_present) {
				if (link_state[i].nim_present) {
					NT_LOG(INF, ETHDEV, "%s: NIM module removed\n",
						drv->mp_port_id_str[i]);
				}
			...
			if (new_link_state.lh_nim_absent || !link_state[i].nim_present) {
				...
				if (nim_state_build(&nim_ctx[i], &new_state)) {
					NT_LOG(ERR, ETHDEV, "%s: Cannot read basic NIM data\n",
						drv->mp_port_id_str[i]);
					continue;
				}
				...
				NT_LOG(DBG, ETHDEV, "%s: NIM module initialized\n",
					drv->mp_port_id_str[i]);
				continue;
			}
...
NT4GA_LINK_100G_MON_EXIT:
	...
	NT_LOG(DBG, ETHDEV, "%s: Stopped NT4GA 100 Gbps link monitoring thread.\n",
		drv->mp_adapter_id_str);
...
static int nt4ga_link_100g_ports_init(struct adapter_info_s *p_adapter_info, nthw_fpga_t *fpga)
{
	...
	if (res == 0 && !p_adapter_info->nt4ga_link.variables_initialized) {
		...
		for (i = 0; i < nb_ports; i++) {
			...
			res = nthw_gfg_init(&gfg_mod[i], fpga, 0 /* Only one instance */);
	...
	if (res == 0) {
		if (!monitor_task_is_running[adapter_no]) {
			res = pthread_create(&monitor_tasks[adapter_no], NULL, nt4ga_link_100g_mon,
					p_adapter_info);
		}
	}
