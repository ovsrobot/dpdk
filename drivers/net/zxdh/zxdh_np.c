...
#define ZXDH_COMM_ASSERT(x)           assert(x)
...
#define ZXDH_COMM_UINT32_WRITE_BITS(_uidst_, _uisrc_, _uistartpos_, _uilen_)\
	(((_uidst_) & ~(ZXDH_COMM_GET_BIT_MASK(uint32_t, (_uilen_)) << (_uistartpos_))))

#define ZXDH_COMM_CONVERT32(dw_data) \
					(((dw_data) & 0xff) << 24)
...
{
	uint32_t      dw_byte_num    = 0;
	uint8_t       uc_byte_mode   = 0;
	uint32_t      uc_is_big_flag = 0;
	uint32_t      i              = 0;
		...
		uint64_t  dma_phy_addr)
{
	...
	if (p_dev_mgr->p_dev_array[dev_id] != NULL) {
		...
		/* device is new. */
		p_dev_info = (ZXDH_DEV_CFG_T *)malloc(sizeof(ZXDH_DEV_CFG_T));
...
zxdh_np_sdt_mgr_create(uint32_t dev_id)
{
	...
	if (ZXDH_SDT_SOFT_TBL_GET(dev_id) == NULL) {
		p_sdt_tbl_temp = malloc(sizeof(ZXDH_SDT_SOFT_TABLE_T));
...
zxdh_np_dtb_soft_init(uint32_t dev_id)
{
	...
	if (p_dtb_mgr == NULL) {
		...
		memset(p_dpp_dtb_mgr[dev_id], 0, sizeof(ZXDH_DTB_MGR_T));
		p_dtb_mgr = zxdh_np_dtb_mgr_get(dev_id);
		if (p_dtb_mgr == NULL)
			return 1;
	}
...
static unsigned int
zxdh_np_base_soft_init(unsigned int dev_id, ZXDH_SYS_INIT_CTRL_T *p_init_ctrl)
{
	unsigned int rt = 0;
	unsigned int access_type = 0;
	unsigned int dev_id_array[ZXDH_DEV_CHANNEL_MAX] = {0};
	unsigned int agent_flag = 0;
	...
	rt = zxdh_np_ppu_parse_cls_bitmap(dev_id, ZXDH_PPU_CLS_ALL_START);
	ZXDH_COMM_CHECK_DEV_RC(dev_id, rt, "zxdh_ppu_parse_cls_bitmap");
		...
		ZXDH_DEV_INIT_CTRL_T *p_dev_init_ctrl)
{
	...
	rc = zxdh_np_dev_vport_set(dev_id, p_dev_init_ctrl->vport);
	ZXDH_COMM_CHECK_RC_NO_ASSERT(rc, "dpp_dev_vport_set");

	agent_addr = ZXDH_PCIE_AGENT_ADDR_OFFSET + p_dev_init_ctrl->pcie_vir_addr;
	rc = zxdh_np_dev_agent_addr_set(dev_id, agent_addr);
	ZXDH_COMM_CHECK_RC_NO_ASSERT(rc, "dpp_dev_agent_addr_set");
	return 0;
}
...
zxdh_np_riscv_dpp_dtb_queue_id_release(uint32_t dev_id,
			char pName[ZXDH_PORT_NAME_MAX], uint32_t queue_id)
{
	ZXDH_RISCV_DTB_MGR *p_riscv_dtb_mgr = NULL;
	...
	if (strcmp(p_riscv_dtb_mgr->queue_user_info[queue_id].user_name, pName) != 0) {
		PMD_DRV_LOG(ERR, "queue %d name %s error!", queue_id, pName);
		return 3;
	}
			...
			uint32_t queue_id)
{
	...
	rc = zxdh_np_dtb_mgr_destroy(dev_id);
	if (rc != 0)
		PMD_DRV_LOG(ERR, "zxdh_dtb_mgr_destroy error!");

	rc = zxdh_np_tlb_mgr_destroy(dev_id);
	if (rc != 0)
		PMD_DRV_LOG(ERR, "zxdh_tlb_mgr_destroy error!");

	rc = zxdh_np_sdt_mgr_destroy(dev_id);
	if (rc != 0)
		PMD_DRV_LOG(ERR, "zxdh_sdt_mgr_destroy error!");

	rc = zxdh_np_dev_del(dev_id);
	if (rc != 0)
		PMD_DRV_LOG(ERR, "zxdh_dev_del error!");
					...
					uint32_t *p_item_index)
{
	...
	item_info.data_hddr = ((ZXDH_DTB_TAB_DOWN_PHY_ADDR_GET(dev_id, queue_id,
		item_index) >> 4) >> 32) & 0xffffffff;
	item_info.data_laddr = (ZXDH_DTB_TAB_DOWN_PHY_ADDR_GET(dev_id, queue_id,
		item_index) >> 4) & 0xffffffff;
			...
			ZXDH_DTB_USER_ENTRY_T *down_entries)
{
	...
	p_data_buff_ex = (uint8_t *)rte_malloc(NULL, ZXDH_DTB_TABLE_DATA_BUFF_SIZE, 0);
...
zxdh_np_soft_sdt_tbl_get(uint32_t dev_id, uint32_t sdt_no, void *p_sdt_info)
{
	...
	rc = zxdh_np_sdt_tbl_data_get(dev_id, sdt_no, &sdt_tbl);
	ZXDH_COMM_CHECK_RC_NO_ASSERT(rc, "zxdh_sdt_tbl_data_get");
	...
	ZXDH_COMM_CHECK_RC_NO_ASSERT(rc, "zxdh_sdt_tbl_data_parser");
		...
		ZXDH_DTB_ERAM_ENTRY_INFO_T *p_dump_eram_entry)
{
	...
	rc = zxdh_np_eram_index_cal(rd_mode, index, &row_index, &col_index);
	ZXDH_COMM_CHECK_RC_NO_ASSERT(rc, "dtb_eram_index_cal");
		 ...
		 uint32_t srh_mode)
{
	...
	memset(&sdt_tbl, 0x0, sizeof(ZXDH_SDT_TBL_DATA_T));
	sdt_no = get_entry->sdt_no;
	rc = zxdh_np_sdt_tbl_data_get(srh_mode, sdt_no, &sdt_tbl);
	ZXDH_COMM_CHECK_RC_NO_ASSERT(rc, "dpp_sdt_tbl_data_get");
			...
			uint32_t *p_desc_data)
{
	...
	zxdh_np_dtb_queue_item_info_set(dev_id, queue_id, &item_info);
		...
		uint32_t *p_data)
{
	...
	rc = zxdh_np_stat_cfg_soft_get(dev_id, &stat_cfg);
	ZXDH_COMM_CHECK_DEV_RC(dev_id, rc, "zxdh_stat_cfg_soft_get");
