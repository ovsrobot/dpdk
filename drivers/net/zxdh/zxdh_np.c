...
static ZXDH_DTB_FIELD_T g_dtb_eram_dump_cmd_info[] = {
	{"valid", 127, 1},
	{"up_type", 126, 2},
	{"base_addr", 106, 19},
	{"tb_depth", 83, 20},
...
static ZXDH_DTB_FIELD_T g_dtb_ddr_dump_cmd_info[] = {
	{"valid", 127, 1},
	{"up_type", 126, 2},
	{"base_addr", 117, 30},
	{"tb_depth", 83, 20},
...
static ZXDH_DTB_FIELD_T g_dtb_zcam_dump_cmd_info[] = {
	{"valid", 127, 1},
	{"up_type", 126, 2},
	{"zgroup_id", 124, 2},
	{"zblock_id", 122, 3},
...
static ZXDH_DTB_FIELD_T g_dtb_etcam_dump_cmd_info[] = {
	{"valid", 127, 1},
	{"up_type", 126, 2},
	{"block_sel", 124, 3},
	{"addr", 121, 9},
...
static ZXDH_DTB_TABLE_T g_dpp_dtb_dump_info[] = {
	{
		"eram",
		ZXDH_DTB_DUMP_ERAM,
		6,
...
zxdh_comm_liststack_free(ZXDH_LISTSTACK_MANAGER *p_list, uint32_t index)
{
	...
	if (dw_index >= p_list->capacity) {
		PMD_DRV_LOG(INFO, "dw_index is invalid");
...
{
	if (p_list == NULL) {
		PMD_DRV_LOG(INFO, "p_list point null");
		return ZXDH_LIST_STACK_POINT_NULL;
	}
	rte_free(p_list);
		...
		uint64_t  dma_phy_addr)
{
	...
	for (i = 0; i < ZXDH_HASH_FUNC_ID_NUM; i++)
		zxdh_np_comm_spinlock_create(&p_dev_info->hash_spinlock[i]);
					...
					uint32_t element_id)
{
	...
	while (!success_flag) {
		...
		if (rd_cnt > ZXDH_DTB_DOWN_OVER_TIME) {
			PMD_DRV_LOG(ERR, "down queue %d item %d overtime!", queue_id, element_id);
						...
						ZXDH_DTB_ENTRY_T *p_dtb_one_entry)
{
	...
	zxdh_np_comm_spinlock_lock(p_hash_spinlock);
	...
	if (rc == ZXDH_RBT_RC_SRHFAIL) {
		...
		zxdh_np_comm_spinlock_unlock(p_hash_spinlock);
		return ZXDH_HASH_RC_DEL_SRHFAIL;
	}
					...
					uint32_t element_id)
{
	...
	while (!success_flag) {
		...
		if (rd_cnt > ZXDH_DTB_DUMP_OVER_TIME) {
			PMD_DRV_LOG(ERR, "dump queue %d item %d overtime!", queue_id, element_id);
...
zxdh_np_se_res_init(uint32_t dev_id, uint32_t type)
{
	...
	p_se_res = (ZXDH_APT_SE_RES_T *)zxdh_np_dev_get_se_res_ptr(dev_id, type);
	if (!p_se_res->valid) {
		PMD_DRV_LOG(ERR, "dev_id[0x%x],se_type[0x%x] invlaid!", dev_id, type);
		return ZXDH_ERR;
	}
				...
				uint32_t *p_data)
{
	...
	if ((index >> (ZXDH_STAT_128_MODE - rd_mode)) < ppu_eram_depth) {
		...
		PMD_DRV_LOG(ERR, "not have ddr stat.");
	}
