...
static int zxdh_set_link_status(struct rte_eth_dev *dev, uint8_t link_status)
{
	...
	if (link_status == curr_link_status) {
		PMD_DRV_LOG(INFO, "curr_link_status %u", curr_link_status);
		return 0;
	}
	...
	if (mask & RTE_ETH_VLAN_FILTER_MASK) {
		if (rxmode->offloads & RTE_ETH_RX_OFFLOAD_VLAN_FILTER) {
			if (hw->is_pf) {
				...
				if (ret) {
					PMD_DRV_LOG(ERR, "port %d vlan filter offload set failed",
						hw->vport.vfid);
					return -EAGAIN;
				}
			} else {
	...
	if (mask & RTE_ETH_VLAN_STRIP_MASK) {
		if (rxmode->offloads & RTE_ETH_RX_OFFLOAD_VLAN_STRIP) {
			if (hw->is_pf) {
				...
				ret = zxdh_set_port_attr(hw->vport.vfid, &port_attr);
				if (ret) {
					PMD_DRV_LOG(ERR, "port %d vlan strip offload set failed",
						hw->vport.vfid);
					return -EAGAIN;
				}
			} else {
