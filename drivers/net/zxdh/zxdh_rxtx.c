...
static void
zxdh_refill_desc_unwrap(struct zxdh_virtqueue *vq, struct rte_mbuf **cookie,
		uint16_t nb_pkts)
{
	struct zxdh_vring_packed_desc *start_dp = vq->vq_packed.ring.desc;
...
static inline int
zxdh_init_mbuf(struct rte_mbuf *rxm, uint16_t len,
...
uint16_t
zxdh_recv_single_pkts(void *rx_queue, struct rte_mbuf **rcv_pkts, uint16_t nb_pkts)
{
	struct zxdh_virtnet_rx *rxvq = rx_queue;
	struct zxdh_virtqueue *vq = rxvq->vq;
	struct zxdh_hw *hw = vq->hw;
