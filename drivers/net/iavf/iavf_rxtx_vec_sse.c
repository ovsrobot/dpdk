		   ...
		   uint16_t nb_pkts, uint8_t *split_packet)
{
	...
	for (pos = 0, nb_pkts_recd = 0; pos < nb_pkts;
		...
		/* A.1 load desc[3] */
		descs[3] = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL
			(rxdp + 3));
		...
		/* A.1 load desc[2-0] */
		descs[2] = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL
			(rxdp + 2));
		rte_compiler_barrier();
		descs[1] = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL
			(rxdp + 1));
		rte_compiler_barrier();
		descs[0] = _mm_loadu_si128((__m128i *)RTE_IGNORE_CAST_QUAL
			(rxdp));
			    ...
			    uint16_t nb_pkts, uint8_t *split_packet)
{
	...
	for (pos = 0, nb_pkts_recd = 0; pos < nb_pkts;
		...
		if (offloads & RTE_ETH_RX_OFFLOAD_RSS_HASH ||
			...
			descs_bh[3] = _mm_load_si128((void *)RTE_IGNORE_CAST_QUAL
				(&rxdp[3].wb.status_error1));
			rte_compiler_barrier();
			descs_bh[2] = _mm_load_si128((void *)RTE_IGNORE_CAST_QUAL
				(&rxdp[2].wb.status_error1));
			rte_compiler_barrier();
			descs_bh[1] = _mm_load_si128((void *)RTE_IGNORE_CAST_QUAL
				(&rxdp[1].wb.status_error1));
			rte_compiler_barrier();
			descs_bh[0] = _mm_load_si128((void *)RTE_IGNORE_CAST_QUAL
