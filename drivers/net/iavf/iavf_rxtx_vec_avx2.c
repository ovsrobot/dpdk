...
#define IAVF_DESCS_PER_LOOP_AVX 8
	...
	for (i = 0, received = 0; i < nb_pkts;
		...
		const __m128i raw_desc7 = _mm_load_si128((void *)RTE_IGNORE_CAST_QUAL
				(rxdp + 7));
		rte_compiler_barrier();
		const __m128i raw_desc6 = _mm_load_si128((void *)RTE_IGNORE_CAST_QUAL
				(rxdp + 6));
		rte_compiler_barrier();
		const __m128i raw_desc5 = _mm_load_si128((void *)RTE_IGNORE_CAST_QUAL
				(rxdp + 5));
		rte_compiler_barrier();
		const __m128i raw_desc4 = _mm_load_si128((void *)RTE_IGNORE_CAST_QUAL
				(rxdp + 4));
		rte_compiler_barrier();
		const __m128i raw_desc3 = _mm_load_si128((void *)RTE_IGNORE_CAST_QUAL
				(rxdp + 3));
		rte_compiler_barrier();
		const __m128i raw_desc2 = _mm_load_si128((void *)RTE_IGNORE_CAST_QUAL
				(rxdp + 2));
		rte_compiler_barrier();
		const __m128i raw_desc1 = _mm_load_si128((void *)RTE_IGNORE_CAST_QUAL
				(rxdp + 1));
		rte_compiler_barrier();
		const __m128i raw_desc0 = _mm_load_si128((void *)RTE_IGNORE_CAST_QUAL
				(rxdp + 0));
		...
		if (offload) {
			...
			if (offloads & RTE_ETH_RX_OFFLOAD_RSS_HASH ||
				...
				const __m128i raw_desc_bh7 =
					_mm_load_si128((void *)RTE_IGNORE_CAST_QUAL
	  ...
	  struct rte_mbuf *pkt, uint64_t flags, bool offload)
{
	...
	_mm_store_si128((__m128i *)RTE_IGNORE_CAST_QUAL(txdp), descriptor);
}
	 ...
	 struct rte_mbuf **pkt, uint16_t nb_pkts,  uint64_t flags, bool offload)
{
	...
	for (; nb_pkts > 3; txdp += 4, pkt += 4, nb_pkts -= 4) {
		...
		_mm256_store_si256((void *)RTE_IGNORE_CAST_QUAL(txdp + 2), desc2_3);
		_mm256_store_si256((void *)RTE_IGNORE_CAST_QUAL(txdp), desc0_1);
	}
