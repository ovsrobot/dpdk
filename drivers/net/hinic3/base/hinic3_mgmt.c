...
int
hinic3_msg_to_mgmt_sync(struct hinic3_hwdev *hwdev, enum hinic3_mod_type mod, uint16_t cmd,
			void *buf_in, uint16_t in_size, void *buf_out, uint16_t *out_size,
			uint32_t timeout)
{
	int err;
	...
	err = hinic3_send_mbox_to_mgmt(hwdev, mod, cmd, HINIC3_MGMT_SRC_ID, buf_in, in_size,
...
send_mgmt_ack(struct hinic3_msg_pf_to_mgmt *pf_to_mgmt,
	      enum hinic3_mod_type mod, uint16_t cmd, void *buf_in, uint16_t in_size,
	      uint16_t msg_id)
{
	uint16_t buf_size;
