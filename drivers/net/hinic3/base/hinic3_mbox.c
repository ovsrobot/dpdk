		...
		struct hinic3_handler_info *handler_info, struct mbox_msg_info *msg_info)
{
	...
	while (!(HINIC3_MSG_HEADER_GET(header, LAST))) {
		...
		err = send_mbox_seg(func_to_func, header, handler_info->dst_func, msg_seg,
				    seg_len, msg_info);
		if (err) {
			PMD_DRV_LOG(ERR,
			...
			struct hinic3_handler_info *handler_info, struct mbox_msg_info *msg_info)
{
	...
	/* Send a message. */
	err = send_tlp_mbox_seg(func_to_func, header, handler_info->dst_func, msg_seg,
				handler_info->in_size, msg_info);
	if (err) {
		    ...
		    struct hinic3_handler_info *handler_info, uint32_t timeout)
{
	...
	mbox_rsp_idx = (uint16_t)hinic3_mbox_get_index(handler_info->dst_func);
