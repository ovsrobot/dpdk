			 ...
			 const uint8_t *mac_addr, uint16_t vlan_id, uint16_t func_id)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_SET_MAC, &mac_info,
			 ...
			 const uint8_t *mac_addr, uint16_t vlan_id, uint16_t func_id)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_DEL_MAC, &mac_info,
				     sizeof(mac_info), &mac_info, &out_size);
	if (err || !out_size ||
...
hinic3_clear_phy_port_stats(struct hinic3_hwdev *hwdev)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_HILINK, MAG_CMD_CLR_PORT_STAT, &port_stats,
				...
				uint8_t ipv6_en, uint8_t lro_max_pkt_len)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_CFG_RX_LRO, &lro_cfg,
			      ...
			      enum hinic3_link_follow_status status)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_HILINK, MAG_CMD_SET_LINK_FOLLOW, &follow,
