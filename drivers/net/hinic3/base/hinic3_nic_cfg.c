...
hinic3_set_ci_table(struct hinic3_hwdev *hwdev, struct hinic3_sq_attr *attr)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_SQ_CI_ATTR_SET,
			 ...
			 const uint8_t *mac_addr, uint16_t vlan_id, uint16_t func_id)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_SET_MAC,
...
int
hinic3_del_mac(struct hinic3_hwdev *hwdev,
			 const uint8_t *mac_addr, uint16_t vlan_id, uint16_t func_id)
{
	struct hinic3_port_mac_set mac_info;
	uint16_t out_size = sizeof(mac_info);
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_DEL_MAC,
			&mac_info, sizeof(mac_info), &mac_info, &out_size);
	if (err || !out_size ||
		...
		uint8_t *new_mac, uint16_t vlan_id, uint16_t func_id)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_UPDATE_MAC,
				     &mac_info, sizeof(mac_info), &mac_info, &out_size);
...
hinic3_get_default_mac(struct hinic3_hwdev *hwdev, uint8_t *mac_addr, int ether_len)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_GET_MAC, &mac_info,
...
hinic3_config_vlan(struct hinic3_hwdev *hwdev, uint8_t opcode, uint16_t vlan_id, uint16_t func_id)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_CFG_FUNC_VLAN,
				     &vlan_info, sizeof(vlan_info), &vlan_info, &out_size);
...
hinic3_set_vport_enable(struct hinic3_hwdev *hwdev, bool enable)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_SET_VPORT_ENABLE,
...
hinic3_flush_qps_res(struct hinic3_hwdev *hwdev)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_CLEAR_QP_RESOURCE,
				     &sq_res, sizeof(sq_res), &sq_res, &out_size);
...
hinic3_cfg_hw_pause(struct hinic3_hwdev *hwdev, uint8_t opcode, struct nic_pause_config *nic_pause)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_CFG_PAUSE_INFO,
				     &pause_info, sizeof(pause_info), &pause_info, &out_size);
...
hinic3_get_vport_stats(struct hinic3_hwdev *hwdev, struct hinic3_vport_stats *stats)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_GET_VPORT_STAT,
				     &stats_info, sizeof(stats_info), &vport_stats, &out_size);
...
hinic3_get_phy_port_stats(struct hinic3_hwdev *hwdev, struct mag_phy_port_stats *stats)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_HILINK, MAG_CMD_GET_PORT_STAT, &stats_info,
...
hinic3_clear_vport_stats(struct hinic3_hwdev *hwdev)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_CLEAN_VPORT_STAT,
...
hinic3_clear_phy_port_stats(struct hinic3_hwdev *hwdev)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_HILINK, MAG_CMD_CLR_PORT_STAT,
				&port_stats, sizeof(port_stats), &port_stats, &out_size);
	if (err || !out_size || port_stats.head.status) {
		PMD_DRV_LOG(ERR,
			    "Failed to get port statistics, err: %d, status: 0x%x, out size: 0x%x",
			    err, port_stats.head.status, out_size);
			  ...
			  struct hinic3_func_tbl_cfg *cfg)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_SET_FUNC_TBL,
				&cmd_func_tbl, sizeof(cmd_func_tbl), &cmd_func_tbl, &out_size);
...
nic_feature_nego(struct hinic3_hwdev *hwdev, uint8_t opcode, uint64_t *s_feature, uint16_t size)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_FEATURE_NEGO,
				     &feature_nego, sizeof(feature_nego), &feature_nego, &out_size);
...
hinic3_vf_func_init(struct hinic3_hwdev *hwdev)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_VF_REGISTER,
			&register_info, sizeof(register_info), &register_info, &out_size);
...
hinic3_vf_func_free(struct hinic3_hwdev *hwdev)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_VF_REGISTER,
			&unregister, sizeof(unregister), &unregister, &out_size);
...
hinic3_set_rx_mode(struct hinic3_hwdev *hwdev, uint32_t enable)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_SET_RX_MODE,
			&rx_mode_cfg, sizeof(rx_mode_cfg), &rx_mode_cfg, &out_size);
...
hinic3_set_rx_vlan_offload(struct hinic3_hwdev *hwdev, uint8_t en)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_SET_RX_VLAN_OFFLOAD,
			&vlan_cfg, sizeof(vlan_cfg), &vlan_cfg, &out_size);
...
hinic3_set_vlan_filter(struct hinic3_hwdev *hwdev, uint32_t vlan_filter_ctrl)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_SET_VLAN_FILTER_EN,
			&vlan_filter, sizeof(vlan_filter), &vlan_filter, &out_size);
				...
				uint8_t ipv6_en, uint8_t lro_max_pkt_len)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_CFG_RX_LRO,
			&lro_cfg, sizeof(lro_cfg), &lro_cfg, &out_size);
...
hinic3_set_rx_lro_timer(struct hinic3_hwdev *hwdev, uint32_t timer_value)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_CFG_LRO_TIMER,
				&lro_timer, sizeof(lro_timer), &lro_timer, &out_size);
			...
			uint32_t lro_max_pkt_len)
{
	...
	PMD_DRV_LOG(DEBUG, "Set LRO max coalesce packet size to %uK",
		    lro_max_pkt_len);
...
hinic3_rss_template_alloc(struct hinic3_hwdev *hwdev)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_RSS_TEMP_MGR,
			&template_mgmt, sizeof(template_mgmt), &template_mgmt, &out_size);
...
hinic3_rss_cfg_hash_key(struct hinic3_hwdev *hwdev, uint8_t opcode, uint8_t *key, uint16_t key_size)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_CFG_RSS_HASH_KEY,
			&hash_key, sizeof(hash_key), &hash_key, &out_size);
...
hinic3_get_rss_type(struct hinic3_hwdev *hwdev, struct hinic3_rss_type *rss_type)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_GET_RSS_CTX_TBL,
			&ctx_tbl, sizeof(ctx_tbl), &ctx_tbl, &out_size);
...
hinic3_rss_cfg_hash_engine(struct hinic3_hwdev *hwdev, uint8_t opcode, uint8_t *type)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_CFG_RSS_HASH_ENGINE,
			&hash_type, sizeof(hash_type), &hash_type, &out_size);
...
hinic3_rss_cfg(struct hinic3_hwdev *hwdev, uint8_t rss_en, uint8_t tc_num, uint8_t *prio_tc)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_RSS_CFG, &rss_cfg,
...
hinic3_vf_get_default_cos(struct hinic3_hwdev *hwdev, uint8_t *cos_id)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_VF_COS, &vf_dcb,
			 ...
			 uint8_t pkt_type, uint16_t queue_id, uint8_t en)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_SET_FDIR_STATUS,
			&ethertype_cmd, sizeof(ethertype_cmd), &ethertype_cmd, &out_size);
		     ...
		     uint8_t tcam_rule_type)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_ADD_TC_FLOW,
			&tcam_cmd, sizeof(tcam_cmd), &tcam_cmd, &out_size);
...
hinic3_del_tcam_rule(struct hinic3_hwdev *hwdev, uint32_t index, uint8_t tcam_rule_type)
{
	...
	/* Synchronize the information to the management module. */
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_DEL_TC_FLOW,
			&tcam_cmd, sizeof(tcam_cmd), &tcam_cmd, &out_size);
	if (err || tcam_cmd.msg_head.status || !out_size) {
		PMD_DRV_LOG(ERR,
...
hinic3_cfg_tcam_block(struct hinic3_hwdev *hwdev, uint8_t alloc_en, uint16_t *index)
{
	...
	/* Synchronize the information to the management module. */
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_CFG_TCAM_BLOCK,
...
hinic3_flush_tcam_rule(struct hinic3_hwdev *hwdev)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_FLUSH_TCAM,
...
hinic3_set_fdir_tcam_rule_filter(struct hinic3_hwdev *hwdev, bool enable)
{
	...
	/* Synchronize the information to the management module. */
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_L2NIC, HINIC3_NIC_CMD_ENABLE_TCAM,
			      ...
			      enum hinic3_link_follow_status status)
{
	...
	err = hinic3_msg_to_mgmt_sync(hwdev, HINIC3_MOD_HILINK, MAG_CMD_SET_LINK_FOLLOW,
				&follow, sizeof(follow), &follow, &out_size);
