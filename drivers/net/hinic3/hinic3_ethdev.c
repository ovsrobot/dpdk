...
hinic3_dev_configure(struct rte_eth_dev *dev)
{
	...
	if (nic_dev->num_sqs > nic_dev->max_sqs ||
		...
		PMD_DRV_LOG(ERR,
			    "num_sqs: %d or num_rqs: %d larger than max_sqs: %d or max_rqs: %d",
	...
	if (HINIC3_MAX_RX_PKT_LEN(dev->data->dev_conf.rxmode) <
		...
		PMD_DRV_LOG(ERR,
			    "Max rx pkt len out of range, max_rx_pkt_len: %d, expect between %d and %d",
			    HINIC3_MAX_RX_PKT_LEN(dev->data->dev_conf.rxmode),
...
hinic3_dev_rx_queue_start(struct rte_eth_dev *dev, uint16_t rq_id)
{
	...
	rc = hinic3_enable_rxq_fdir_filter(dev, (uint32_t)rq_id, (uint32_t)true);
	if (rc) {
...
hinic3_dev_rx_queue_stop(struct rte_eth_dev *dev, uint16_t rq_id)
{
	...
	rc = hinic3_enable_rxq_fdir_filter(dev, (uint32_t)rq_id, (uint32_t)false);
	if (rc) {
...
hinic3_disable_queue_intr(struct rte_eth_dev *dev)
{
	...
	for (i = 0; i < nic_dev->num_rqs; i++) {
		...
		hinic3_set_msix_state(nic_dev->hwdev, (uint16_t)msix_intr,
				      HINIC3_MSIX_DISABLE);
...
hinic3_vlan_offload_set(struct rte_eth_dev *dev, int mask)
{
	...
	if (mask & RTE_ETH_VLAN_FILTER_MASK) {
		...
		if (err) {
			PMD_DRV_LOG(ERR,
				    "%s vlan filter failed, device: %s, port_id: %d, err: %d",
				    on ? "Enable" : "Disable",
				    nic_dev->dev_name, dev->data->port_id, err);
			return err;
		}
	...
	if (mask & RTE_ETH_VLAN_STRIP_MASK) {
		...
		if (err) {
			PMD_DRV_LOG(ERR,
				    "%s vlan strip failed, device: %s, port_id: %d, err: %d",
				    on ? "Enable" : "Disable",
				    nic_dev->dev_name, dev->data->port_id, err);
			return err;
		}
