 ...
 */
static inline void
hinic3_update_rq_hw_pi(struct hinic3_rxq *rxq, uint16_t pi)
{
	*rxq->pi_virt_addr =
		(uint16_t)rte_cpu_to_be_16((pi & rxq->q_mask) << rxq->wqe_type);
}
...
hinic3_rx_fill_buffers(struct hinic3_rxq *rxq)
{
	...
	if (likely(i > 0)) {
		...
		hinic3_write_db(rxq->db_addr, rxq->q_id, 0, RQ_CFLAG_DP,
				(uint16_t)(rxq->next_to_update << rxq->wqe_type));
...
hinic3_rx_queue_release_mbufs(struct hinic3_rxq *rxq)
{
	...
	hinic3_update_rq_local_ci(rxq, (uint16_t)nr_released);
}
