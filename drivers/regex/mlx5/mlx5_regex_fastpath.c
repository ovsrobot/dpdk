...
static inline void
prep_nop_regex_wqe_set(struct mlx5_regex_priv *priv, struct mlx5_regex_hw_qp *qp,
		       struct rte_regex_ops *op, struct mlx5_regex_job *job,
		       size_t pi, struct mlx5_klm *klm)
{
...
prep_regex_umr_wqe_set(struct mlx5_regex_priv *priv, struct mlx5_regex_qp *qp,
	 struct mlx5_regex_hw_qp *qp_obj, struct rte_regex_ops **op, size_t nb_ops)
{
	...
	while (left_ops--) {
		 ...
		 */
		prep_nop_regex_wqe_set(priv, qp_obj, op[left_ops], job, pi, &klm);
	}
	/*
			  ...
			  struct rte_regex_ops **ops, uint16_t nb_ops)
{
	...
	while ((hw_qpid = ffs(queue->free_qps))) {
		...
		if (nb_desc) {
			...
			prep_regex_umr_wqe_set(priv, queue, qp_obj, ops, nb_desc);
		      ...
		      struct rte_regex_ops **ops, uint16_t nb_ops)
{
	...
	while ((hw_qpid = ffs(queue->free_qps))) {
		...
		while (get_free(qp_obj, priv->has_umr)) {
			job_id = job_id_get(hw_qpid, qp_size_get(qp_obj), qp_obj->pi);
			prep_one(priv, queue, qp_obj, ops[i], &queue->jobs[job_id]);
			i++;
		      ...
		      struct rte_regex_ops **ops, uint16_t nb_ops)
{
	...
	while ((cqe = poll_one(cq))) {
		...
		while (qp_obj->ci != wq_counter) {
			...
			uint32_t job_id = job_id_get(hw_qpid, qp_size_get(qp_obj),
						     qp_obj->ci);
