...
mlx5_compress_init_sq(struct mlx5_compress_qp *qp)
{
	...
	for (i = 0; i < qp->entries_n; ++i, ++wqe) {
		wqe->sq_ds = rte_cpu_to_be_32((qp->sq.sq->id << 8) | 4u);
		wqe->flags = RTE_BE32(MLX5_COMP_ALWAYS <<
				      MLX5_COMP_MODE_OFFSET);
		wqe->opaque_lkey = rte_cpu_to_be_32(qp->opaque_mr.lkey);
			    ...
			    uint16_t nb_ops)
{
	...
	if (remain < nb_ops) {
		ops[remain]->status = RTE_COMP_OP_STATUS_NOT_PROCESSED;
		nb_ops = remain;
	} else {
		remain = nb_ops;
	}
	...
	do {
		...
		if (unlikely(op->op_type != RTE_COMP_OP_STATELESS ||
		    (xform->type == RTE_COMP_COMPRESS && op->flush_flag <
		     RTE_COMP_FLUSH_FULL) || (mlx5_compress_dseg_set(qp,
					&wqe->gather, op->m_src, op->src.offset,
					       op->src.length) == UINT32_MAX) ||
		    (mlx5_compress_dseg_set(qp, &wqe->scatter, op->m_dst,
					    op->dst.offset,
					    rte_pktmbuf_pkt_len(op->m_dst) -
					    op->dst.offset) == UINT32_MAX))) {
