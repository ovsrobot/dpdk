...
zsda_sm4_key_expansion(unsigned int sk[32], const uint8_t key[16])
{
	...
	mk[0] = rte_be_to_cpu_32(*(uint32_t *)&key[0]);
	mk[1] = rte_be_to_cpu_32(*(uint32_t *)&key[4]);
	mk[2] = rte_be_to_cpu_32(*(uint32_t *)&key[8]);
	mk[3] = rte_be_to_cpu_32(*(uint32_t *)&key[12]);
	...
	for (; i < 32; i++) {
		k[i + 4] = k[i] ^
			   (sm4_cal_ci_rk(k[i + 1] ^ k[i + 2] ^ k[i + 3] ^ ck[i]));
		sk[i] = k[i + 4];
	}
}
