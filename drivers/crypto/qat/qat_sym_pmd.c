		...
		struct qat_dev_cmd_param *qat_dev_cmd_param __rte_unused)
{
	...
	snprintf(memz_name, RTE_CRYPTODEV_NAME_MAX_LEN,
			"DRIVER_SYM_%d_MEMZONE_GEN_%d",
			qat_sym_driver_id, qat_pci_dev->qat_dev_gen);
	...
	internals->mz = rte_memzone_lookup(memz_name);
	...
	if (internals->mz == NULL) {
		QAT_LOG(DEBUG,
			"Error allocating memzone, destroying PMD for %s",
			name);
