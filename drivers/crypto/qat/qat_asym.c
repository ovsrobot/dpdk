		...
		struct rte_mempool *mempool)
{
	...
	if (rte_mempool_get(mempool, (void *)&qat_session)) {
		QAT_LOG(ERR, "Cannot allocate asymmetric crypto session");
		return -ENOMEM;
	}
	...
	if (ret) {
		...
		rte_mempool_put(mempool, (void *)&qat_session);
		return ret;
	}

	set_asym_session_private_data(session, dev->driver_id,
		(void *)qat_session);
	return 0;
}
		...
		struct rte_cryptodev_asym_session *session)
{
	...
	void *sess_priv = get_asym_session_private_data(session, index);
	struct qat_asym_session *qat_session =
	...
	if (sess_priv) {
		...
		struct rte_mempool *mempool = rte_mempool_from_obj(sess_priv);

		set_asym_session_private_data(session, index, NULL);
