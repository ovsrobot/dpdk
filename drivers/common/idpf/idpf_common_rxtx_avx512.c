...
idpf_singleq_rearm_common(struct idpf_rx_queue *rxq)
{
	...
	for (i = 0; i < IDPF_RXQ_REARM_THRESH;
		...
		_mm512_store_si512(RTE_PTR_DROP_QUALIFIERS(&rxdp->read),
				dma_addr0_3);
		_mm512_store_si512(RTE_PTR_DROP_QUALIFIERS(&(rxdp + 4)->read),
				dma_addr4_7);
	}
...
idpf_singleq_rearm(struct idpf_rx_queue *rxq)
{
	...
	if (cache->len < IDPF_RXQ_REARM_THRESH) {
		...
		if (ret == 0) {
			...
			if (rxq->rxrearm_nb + IDPF_RXQ_REARM_THRESH >=
				...
				for (i = 0; i < IDPF_VPMD_DESCS_PER_LOOP; i++) {
					rxp[i] = &rxq->fake_mbuf;
					_mm_storeu_si128(RTE_PTR_DROP_QUALIFIERS
	...
	for (i = 0; i < IDPF_RXQ_REARM_THRESH / IDPF_DESCS_PER_LOOP_AVX; i++) {
		...
		_mm512_storeu_si512(RTE_PTR_DROP_QUALIFIERS(rxdp), desc0_1);
		_mm512_storeu_si512(RTE_PTR_DROP_QUALIFIERS((rxdp + 2)), desc2_3);
		_mm512_storeu_si512(RTE_PTR_DROP_QUALIFIERS((rxdp + 4)), desc4_5);
		_mm512_storeu_si512(RTE_PTR_DROP_QUALIFIERS((rxdp + 6)), desc6_7);
	...
	if (rte_mempool_get_bulk(rx_bufq->mp,
		...
		if (rx_bufq->rxrearm_nb + IDPF_RXQ_REARM_THRESH >=
			...
			for (i = 0; i < IDPF_VPMD_DESCS_PER_LOOP; i++) {
				...
				_mm_store_si128(RTE_PTR_DROP_QUALIFIERS(&rxdp[i]),
						dma_addr0);
			}
		}
	...
	if (cache->len < IDPF_RXQ_REARM_THRESH) {
		...
		if (ret == 0) {
			...
			if (rx_bufq->rxrearm_nb + IDPF_RXQ_REARM_THRESH >=
				...
				for (i = 0; i < IDPF_VPMD_DESCS_PER_LOOP; i++) {
					rxp[i] = &rx_bufq->fake_mbuf;
					_mm_storeu_si128(RTE_PTR_DROP_QUALIFIERS(&rxdp[i]),
							 dma_addr0);
				}
			}
		rte_atomic_fetch_add_explicit(&rx_bufq->rx_stats.mbuf_alloc_failed,
					...
					uint16_t nb_pkts)
{
	...
	if (nb_commit >= n) {
		...
		idpf_splitq_vtx(RTE_PTR_DROP_QUALIFIERS(txdp), tx_pkts, n - 1,
				cmd_dtype);
		tx_pkts += (n - 1);
		txdp += (n - 1);

		idpf_splitq_vtx1(RTE_PTR_DROP_QUALIFIERS(txdp), *tx_pkts++,
				cmd_dtype);
	...
	idpf_splitq_vtx(RTE_PTR_DROP_QUALIFIERS(txdp), tx_pkts, nb_commit,
			cmd_dtype);
