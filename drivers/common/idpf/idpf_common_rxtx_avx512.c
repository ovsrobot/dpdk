...
idpf_singleq_rearm_common(struct idpf_rx_queue *rxq)
{
	...
	for (i = 0; i < IDPF_RXQ_REARM_THRESH;
		...
		_mm512_store_si512((__m512i *)RTE_IGNORE_CAST_QUAL(&rxdp->read),
				dma_addr0_3);
		_mm512_store_si512((__m512i *)RTE_IGNORE_CAST_QUAL(&(rxdp + 4)->read),
				dma_addr4_7);
	}
...
idpf_singleq_rearm(struct idpf_rx_queue *rxq)
{
	...
	if (cache->len < IDPF_RXQ_REARM_THRESH) {
		...
		if (ret == 0) {
			...
			if (rxq->rxrearm_nb + IDPF_RXQ_REARM_THRESH >=
				...
				for (i = 0; i < IDPF_VPMD_DESCS_PER_LOOP; i++) {
					rxp[i] = &rxq->fake_mbuf;
					_mm_storeu_si128((__m128i *)RTE_IGNORE_CAST_QUAL
	...
	for (i = 0; i < IDPF_RXQ_REARM_THRESH / IDPF_DESCS_PER_LOOP_AVX; i++) {
		...
		_mm512_storeu_si512((void *)RTE_IGNORE_CAST_QUAL(rxdp), desc0_1);
		_mm512_storeu_si512((void *)RTE_IGNORE_CAST_QUAL((rxdp + 2)), desc2_3);
		_mm512_storeu_si512((void *)RTE_IGNORE_CAST_QUAL((rxdp + 4)), desc4_5);
		_mm512_storeu_si512((void *)RTE_IGNORE_CAST_QUAL((rxdp + 6)), desc6_7);
				   ...
				   uint16_t nb_pkts)
{
	...
	for (i = 0, received = 0; i < nb_pkts;
		...
		const __m128i raw_desc7 =
			_mm_load_si128((void *)RTE_IGNORE_CAST_QUAL(rxdp + 7));
		rte_compiler_barrier();
		const __m128i raw_desc6 =
			_mm_load_si128((void *)RTE_IGNORE_CAST_QUAL(rxdp + 6));
		rte_compiler_barrier();
		const __m128i raw_desc5 =
			_mm_load_si128((void *)RTE_IGNORE_CAST_QUAL(rxdp + 5));
		rte_compiler_barrier();
		const __m128i raw_desc4 =
			_mm_load_si128((void *)RTE_IGNORE_CAST_QUAL(rxdp + 4));
		rte_compiler_barrier();
		const __m128i raw_desc3 =
			_mm_load_si128((void *)RTE_IGNORE_CAST_QUAL(rxdp + 3));
		rte_compiler_barrier();
		const __m128i raw_desc2 =
			_mm_load_si128((void *)RTE_IGNORE_CAST_QUAL(rxdp + 2));
		rte_compiler_barrier();
		const __m128i raw_desc1 =
			_mm_load_si128((void *)RTE_IGNORE_CAST_QUAL(rxdp + 1));
		rte_compiler_barrier();
		const __m128i raw_desc0 =
			_mm_load_si128((void *)RTE_IGNORE_CAST_QUAL(rxdp + 0));
	...
	if (rte_mempool_get_bulk(rx_bufq->mp,
		...
		if (rx_bufq->rxrearm_nb + IDPF_RXQ_REARM_THRESH >=
			...
			for (i = 0; i < IDPF_VPMD_DESCS_PER_LOOP; i++) {
				...
				_mm_store_si128((__m128i *)RTE_IGNORE_CAST_QUAL(&rxdp[i]),
						dma_addr0);
			}
		}
	...
	if (cache->len < IDPF_RXQ_REARM_THRESH) {
		...
		if (ret == 0) {
			...
			if (rx_bufq->rxrearm_nb + IDPF_RXQ_REARM_THRESH >=
				...
				for (i = 0; i < IDPF_VPMD_DESCS_PER_LOOP; i++) {
					rxp[i] = &rx_bufq->fake_mbuf;
					_mm_storeu_si128((__m128i *)RTE_IGNORE_CAST_QUAL(&rxdp[i]),
							 dma_addr0);
				}
			}
		rte_atomic_fetch_add_explicit(&rx_bufq->rx_stats.mbuf_alloc_failed,
	  ...
	  struct rte_mbuf *pkt, uint64_t flags)
{
	...
	_mm_storeu_si128((__m128i *)RTE_IGNORE_CAST_QUAL(txdp), descriptor);
}
	 ...
	 struct rte_mbuf **pkt, uint16_t nb_pkts,  uint64_t flags)
{
	...
	for (; nb_pkts > 3; txdp += 4, pkt += 4, nb_pkts -= 4) {
		...
		_mm512_storeu_si512((void *)RTE_IGNORE_CAST_QUAL(txdp), desc0_3);
	}
					...
					uint16_t nb_pkts)
{
	...
	if (nb_commit >= n) {
		...
		idpf_splitq_vtx((void *)RTE_IGNORE_CAST_QUAL(txdp), tx_pkts, n - 1,
				cmd_dtype);
		tx_pkts += (n - 1);
		txdp += (n - 1);

		idpf_splitq_vtx1((void *)RTE_IGNORE_CAST_QUAL(txdp), *tx_pkts++,
				cmd_dtype);
	...
	idpf_splitq_vtx((void *)RTE_IGNORE_CAST_QUAL(txdp), tx_pkts, nb_commit,
			cmd_dtype);
