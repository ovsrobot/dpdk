...
{
	ADAPTER_VALID_OR_ERR_RET(adapter, -EINVAL);
	FUNC_PTR_OR_ERR_RET(adapter->ops->ticks_remaining_get, -ENOTSUP);

	if (ticks_remaining == NULL)
		return -EINVAL;

	return adapter->ops->ticks_remaining_get(adapter, evtim, ticks_remaining);
}
...
static int
swtim_ticks_remaining_get(const struct rte_event_timer_adapter *adapter,
			  const struct rte_event_timer *evtim,
			  uint64_t *ticks_remaining)
{
...
static const struct event_timer_adapter_ops swtim_ops = {
	...
	.ticks_remaining_get = swtim_ticks_remaining_get,
};
