...
#define RX_ADAPTER_INSTNACE_ARRAY "rx_adapter_instance_array"
...
rte_event_eth_rx_adapter_init(void)
{
	...
	if (event_eth_rxa_inst_info == NULL) {
		event_eth_rxa_inst_info =
			rxa_memzone_array_get(RX_ADAPTER_INSTNACE_ARRAY,
...
rxa_memzone_lookup(void)
{
	...
	if (event_eth_rxa_inst_info == NULL) {
		mz = rte_memzone_lookup(RX_ADAPTER_INSTNACE_ARRAY);
		if (mz == NULL)
			return -ENOMEM;
