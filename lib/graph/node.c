...
__rte_node_register(const struct rte_node_register *reg)
{
	...
	if (reg->errs) {
		...
		for (i = 0; i < reg->errs->nb_errors; i++)
			if (rte_strscpy((char *)(uint64_t)node->errs->err_desc[i],
					reg->errs->err_desc[i], RTE_NODE_ERROR_DESC_SIZE) < 0)
				goto free_err;
	}
...
node_clone(struct node *node, const char *name)
{
	...
	if (node->errs) {
		...
		for (i = 0; i < node->errs->nb_errors; i++)
			if (rte_strscpy((char *)(uint64_t)reg->errs->err_desc[i],
					node->errs->err_desc[i], RTE_NODE_ERROR_DESC_SIZE) < 0)
				goto free_err;
	}
