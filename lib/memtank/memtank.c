...
static uint32_t
memobj_align(uint32_t obj_align)
{
	return RTE_MAX(obj_align, alignof(struct memobj));
}
...
init_chunk(struct rte_memtank *mt, struct memchunk *ch)
{
	...
	p = RTE_ALIGN_CEIL(p, mt->obj_align);
...
check_param(const struct rte_memtank_prm *prm)
{
	...
	align = memobj_align(prm->obj_align);
	obj_size = memobj_size(prm->obj_size, align);
	if (obj_size > UINT32_MAX)
		return -EINVAL;
...
rte_memtank_create(const struct rte_memtank_prm *prm)
{
	...
	mt->raw = p;
	mt->obj_align = memobj_align(prm->obj_align);
	mt->chunk_size = memchunk_size(prm->nb_obj_chunk, prm->obj_size,
		mt->obj_align);
	mt->obj_size = memobj_size(prm->obj_size, mt->obj_align);
