		     ...
		     const struct rte_eth_dev_info *info)
{
	...
	mirror->need_lock = !(info->tx_offload_capa & RTE_ETH_TX_OFFLOAD_MT_LOCKFREE);
...
rte_eth_add_mirror(uint16_t port_id, const struct rte_eth_mirror_conf *conf)
{
	...
	if (rte_eal_process_type() == RTE_PROC_PRIMARY) {
		...
		if (conf->flags & RTE_ETH_MIRROR_DIRECTION_INGRESS)
			ret = ethdev_insert_mirror(&dev->rx_mirror, conf, &dev_info);
		if (ret == 0 && (conf->flags & RTE_ETH_MIRROR_DIRECTION_EGRESS))
			ret = ethdev_insert_mirror(&dev->tx_mirror, conf, &dev_info);
	       ...
	       struct rte_eth_mirror *mirror)
{
	...
	if (mirror->need_lock) {
		...
		nsent = rte_eth_tx_burst(mirror->target, txq, tosend, count);
