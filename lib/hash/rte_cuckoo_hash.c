...
rte_hash_create(const struct rte_hash_parameters *params)
{
	...
	if (params == NULL) {
		RTE_LOG_LINE(ERR, HASH, "%s has no parameters", __func__);
		return NULL;
	}
	...
	if ((params->entries > RTE_HASH_ENTRIES_MAX) ||
			...
			(params->key_len == 0)) {
		rte_errno = EINVAL;
		RTE_LOG_LINE(ERR, HASH, "%s has invalid parameters", __func__);
		return NULL;
	}

	if (params->extra_flag & ~RTE_HASH_EXTRA_FLAGS_MASK) {
		rte_errno = EINVAL;
		RTE_LOG_LINE(ERR, HASH, "%s: unsupported extra flags", __func__);
		return NULL;
	}
	...
	if ((params->extra_flag & RTE_HASH_EXTRA_FLAGS_RW_CONCURRENCY) &&
		...
		rte_errno = EINVAL;
		RTE_LOG_LINE(ERR, HASH, "%s: choose rw concurrency or rw concurrency lock free",
			__func__);
		return NULL;
	}
