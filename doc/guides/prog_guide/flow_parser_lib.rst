...
.. note::
   ...
   ``rte_flow_parser_internal.h`` for use by testpmd and DPDK test applications.
   These internal APIs are not part of the stable ABI and may change without
   notice.
...
The internal header ``rte_flow_parser_internal.h`` provides additional
...
These internal APIs support the complete testpmd ``flow`` CLI grammar including
templates, indirect actions, tunnel helpers, and raw encap/decap commands.
They are **not exported** from the shared library and are not part of the
stable ABI.
