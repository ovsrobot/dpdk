...
.. Note::
	...
	When using `use_cni`_, both parameter `xdp_prog`_ and `busy_budget`_ is disabled
	as both of these will be handled by the AF_XDP plugin. Since the
	DPDK application is running in limited privileges so enabling and
	disabling of the promiscuous mode through the DPDK application is
	also not supported.
