...
This second read is important to give the job stats library
feedback of how many packets were processed.
...
To maximize performance, exactly ``MAX_PKT_BURST`` is expected (the target value)
to be read for each ``l2fwd_fwd_job()`` call.
If ``total_nb_rx`` is smaller than target value ``job->period`` will be increased.
If it is greater, the period will be decreased.
...
a static array of destination ports (``l2fwd_dst_ports[]``)
